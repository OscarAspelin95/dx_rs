...
pub async fn connect_db(max_retries: usize) -> Result<Surreal<Client>, DatabaseError> {
    ...
    let db: Surreal<Client> = loop {
        ...
        if retries > max_retries {
            return Err(DatabaseError::DatabaseConnectionTimeoutError());
        }
    ...
    match db.health().await {
        ...
        Err(e) => return Err(DatabaseError::DatabaseUnhealthyError(e.to_string())),
    };
